...
const WhatsAppChat = () => {
  ...
  useEffect(() => {
    const onScroll = () => setShowLauncher(window.scrollY > 300);
    onScroll();
  ...
  return (
    <>
       <AnimatePresence>
         {showLauncher && (
           <motion.div
             ...
             initial={{ opacity: 0, scale: 0.9, y: 88 }}
             animate={{ opacity: 1, scale: 1, y: 0 }}
             exit={{ opacity: 0, scale: 0.9, y: 88 }}
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            className="fixed inset-0 z-50 flex items-center justify-center sm:items-end sm:justify-end sm:pr-6 sm:pb-36"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          >
            <motion.div
              className="w-[92vw] max-w-sm max-h-[85vh] bg-white rounded-2xl shadow-2xl overflow-hidden border border-gray-200 pointer-events-auto sm:w-96 sm:h-[500px]"
